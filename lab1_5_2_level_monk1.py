...
firstSplit = getBestAttribute(m.monk1, m.attributes, available)
available[firstSplit] = False
sets = []
for i in range(0, len(m.attributes[firstSplit].values)):
    sets.append(d.select(m.monk1, m.attributes[firstSplit], m.attributes[firstSplit].values[i]))

printGains(sets, m.attributes)
